...
`requirements.txt` — see `docs/source/developer/automation-design.rst`.
...
every name and no values; `docs/source/components/secrets.rst` is the
inventory. Do not put
...
service passwords. See `docs/source/components/secrets.rst` for the full
inventory and rotation steps.
...
- VM specs are defined in modules/vm_definitions.py from the values in
  `docs/source/reference/proxmox.rst`.
