...
| Play serialisation | `serial: "{{ kubecp_serial | default(1) }}"` in the join play (the default is the effective width; the `kubecp_serial` line in `group_vars/all/main.yml` documents the value but does not reach the keyword) | *(2026-09-10: a 2-wide widening was attempted and reverted after a cold build fataled on it — the observed failure is described in this row.)* One node at a time in the heavy window. The batch-parallel design was rejected on evidence (G3, `agent_plans/ansible-performance.md`): a multi-node join burst saturates repo01's GitLab registry and the shared PVE LVM while every node runs fetch + unpack + image pull + etcd join, and that saturation is what turns one node's CNI image extract into a partial write that RKE2's `.extracted` marker locks in permanently (the kubewk02 portmap FIPS abort). Serial keeps the window single-node and the wall-clock cost is paid once, not N times, because a warm re-run no-ops every role. The bootstrap ordering constraint is unchanged: `kubecp01` starts the cluster alone. 2026-09-10: 2-wide was tried on dedicated hardware and fataled on a 2-node cold build (`kubecp02` "Unable to start service rke2-server") because the joiner's systemd start fires the etcd-member-add *before* the `readyz` poll that follows it — a bootstrap+joiner batch races the bootstrap's API, so one etcd voter at a time remains the control-plane constraint, independent of the I/O era. `-e kubecp_serial=N` (N at least the group size) still recovers an even-sized etcd whose quorum can never be reached one member at a time. See playbooks/kubecp.yml |
