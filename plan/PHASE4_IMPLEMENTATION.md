...
| Minting the token | Root PAT created through `gitlab-rails runner`, then the REST API | The REST API is stable across GitLab versions in a way the Rails models are not. The one Rails call used is the token-creation snippet GitLab documents itself. The PAT is revoked in the role's `always` block, so it does not outlive the run even when the run fails |
