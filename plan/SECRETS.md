...
| **Keycloak itself** | The `admin` account in the **master** realm is local and unfederated. This is why applications and federated users live in `dev-lo` and master holds administrators only | `kv/keycloak` in the vault, and `KEYCLOAK_ADMIN_PASSWORD` in `env.sh` |
