...
| Play serialisation | `serial: 1` | Agents joining do not have etcd's quorum problem, so this is not strictly required. It is kept because three nodes pulling the full image set through one GitLab at once is the other way to make this lab's storage unhappy |
