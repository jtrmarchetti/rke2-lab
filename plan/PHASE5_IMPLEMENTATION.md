...
| Play serialisation | `serial: "{{ kubewk_serial | default(3) }}"` in the join play (the default is the effective width; the `kubewk_serial` line in `group_vars/all/main.yml` documents the value but does not reach the keyword) | *(superseded 2026-09-09: now 3-wide on dedicated hardware; validated on a cold build 2026-09-10 — all three workers joined clean in one batch. See playbooks/kubewk.yml for the current rationale; also note the play-level serial templating trap recorded there.)* The nested-VM-era decision (recorded here for the G3 evidence chain, `agent_plans/ansible-performance.md`) ran joins 1-wide: three workers joining simultaneously saturated repo01's GitLab registry and the shared PVE LVM during fetch + unpack + ~11 image pulls, and that saturation turned kubewk02's CNI image extract into a partial write (portmap FIPS self-hash abort, pod sandbox creation rc=134) that RKE2's `.extracted` marker then locked in permanently. On the current dedicated hardware that I/O path no longer applies: `rke2_publish` pre-mirrors the full image set into repo01's registry before any join, so the join window only pulls from repo01 and each VM unpacks on its own virtual disk. Workers add no etcd membership (kubelet-only), so 3-wide is safe; a cold build 2026-09-10 validated it — all three workers joined clean in one batch while the control plane ran 1-wide (a 2-wide control-plane batch fataled the same build, which is why the control plane was not widened — see the Phase 4 row). A warm re-run no-ops every join role, so the width costs nothing there. The agent Ready waits are still 90×10s=900s per node, which comfortably covers the cilium CNI-installer's 600s startup-probe window. See playbooks/kubewk.yml |
