...
| Play serialisation | `serial: "{{ kubewk_serial | default(3) }}"` in the join play (the default is the effective width; the `kubewk_serial` line in `group_vars/all/main.yml` documents the value but does not reach the keyword) | *(superseded 2026-09-09: now 3-wide on dedicated hardware — see playbooks/kubewk.yml for the current rationale; also note the play-level serial templating trap recorded there.)* One worker at a time in the heavy window. The parallel-join assumption was disproved on evidence (G3, `agent_plans/ansible-performance.md`): three workers joining simultaneously saturate repo01's GitLab registry and the shared PVE LVM during fetch + unpack + ~11 image pulls, and that saturation turned kubewk02's CNI image extract into a partial write (portmap FIPS self-hash abort, pod sandbox creation rc=134). RKE2's `.extracted` marker makes the corruption permanent, so the fix is preventing the burst, not healing it: serial keeps the I/O window single-node, and a warm re-run no-ops every role so the serial costs nothing. The agent Ready waits are already 90×10s=900s per node, which comfortably covers the cilium CNI-installer's 600s startup-probe window. See playbooks/kubewk.yml |
