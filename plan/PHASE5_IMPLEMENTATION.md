...
| Play serialisation | `serial: "{{ kubewk_serial | default(1) }}"` in the join play (the default is the effective width; the `kubewk_serial` line in `group_vars/all/main.yml` documents the value but does not reach the keyword) | *(widening attempt 2026-09-09/10 measured and reverted: 3-wide was SAFE on dedicated hardware but ~2 min SLOWER, so 1-wide stays. See playbooks/kubewk.yml for the current rationale, incl. the play-level serial templating trap.)* The nested-VM-era decision (recorded here for the G3 evidence chain, `agent_plans/ansible-performance.md`) ran joins 1-wide: three workers joining simultaneously saturated repo01's GitLab registry and the shared PVE LVM during fetch + unpack + ~11 image pulls, and that saturation turned kubewk02's CNI image extract into a partial write (portmap FIPS self-hash abort, pod sandbox creation rc=134) that RKE2's `.extracted` marker then locked in permanently. 2026-09-10 widening attempt: 3-wide was SAFE (all three workers joined clean, no symptom — the pre-mirror + per-VM-disk hardware removes the tear risk) but SLOWER: the 3-way apt burst left one worker on 'Ensure base packages are installed' for ~5:25, the worker window went 309s serial → 431s one-batch, and the whole cold build went 48m37s → 51m25s. Serial is both the proven and the faster width, so it stays. A warm re-run no-ops every join role, so the width costs nothing there. The agent Ready waits are still 90×10s=900s per node, which comfortably covers the cilium CNI-installer's 600s startup-probe window. See playbooks/kubewk.yml |
