...
| Play serialisation | `serial: 2` (default), overridable with `-e kubewk_serial=1` | Agents joining do not have etcd's quorum problem, so the joiner's host build and image pulls are free to overlap. One at a time was kept only to stop three nodes pulling the full image set through one GitLab from making the lab's storage unhappy, and to keep a failed join easy to read. Two at a time keeps that blast radius while removing the serial wall; `-e kubewk_serial=1` restores it when the download path or storage is under load. See playbooks/kubewk.yml |
